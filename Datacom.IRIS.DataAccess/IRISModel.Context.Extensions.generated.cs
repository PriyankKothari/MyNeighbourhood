...
namespace Datacom.IRIS.DataAccess
{
    ...
    {
    	   /// <summary>
    	/// (PDF) Extension to only call LoadProperty if the property is null or the property is a collection and the collection is empty.
    	/// </summary>
    	/// <typeparam name="TEntity">Expected type of the Entity</typeparam>
    	/// <param name="context">The ObjectContext on which LoadProperty will be called.</param>
    	/// <param name="selector">Expression that describes the property to be loaded.</param>
    	/// <param name="entity">The entity that the specified property is to be loaded onto.</param>
    	public static void LoadPropertyIfRequired<TEntity>(this ObjectContext context, TEntity entity, System.Linq.Expressions.Expression<Func<TEntity, object>> selector)
    	{
    		if (entity == null || selector == null)
    			return;
    
    		//If the entity is being added LoadProperty will obviously fail
    		IObjectWithChangeTracker trackedEntity = entity as IObjectWithChangeTracker;
    		if (trackedEntity != null && trackedEntity.ChangeTracker.State == ObjectState.Added)
    			return;
    
    		Func<TEntity, object> property = selector.Compile();
    
    		var propertyResolved = property(entity);
    		if (propertyResolved == null || (propertyResolved is System.Collections.ICollection && ((System.Collections.ICollection)propertyResolved).Count == 0))
    			context.LoadProperty<TEntity>(entity, selector);
    	}
    
    	/// <summary>
    	/// (PDF) Extension to call LoadProperty on all members of a collection.
    	/// </summary>
    	/// <typeparam name="TEntity">Expected type of the Entities in the collection</typeparam>
    	/// <param name="context">The ObjectContext on which LoadProperty will be called.</param>
    	/// <param name="selector">Expression that describes the property to be loaded for each entity in the collection.</param>
    	/// <param name="entity">The colection of entities that the specified property is to be loaded onto.</param>
    	public static void LoadPropertiesIfRequired<TEntity>(this ObjectContext context, TrackableCollection<TEntity> entityCollection, System.Linq.Expressions.Expression<Func<TEntity, object>> selector)
    	{
    		if (entityCollection == null || !entityCollection.Any() || selector == null)
    			return;
    
    		foreach (var item in entityCollection)
    		{
    			LoadPropertyIfRequired(context, item, selector);
    		}
    	}
    	...
    	private static string CreateReferenceKeyLookup(string keyMemberName, EntityReference reference, NavigationProperty navigationProperty)
    	{
    		...
    		if (navigationProperty != null)
    		{
    			return String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0}.{1}", navigationProperty.Name, keyMemberName);
    		}
    		else
    		{
    			return String.Format(System.Globalization.CultureInfo.InvariantCulture, "Navigate({0}.{1}).{2}", reference.RelationshipSet.ElementType.FullName, reference.TargetRoleName, keyMemberName);
    		}
    	}
    	...
    	private static void UpdateOriginalValues(ComplexType complexType, string entityTypeName, string propertyPathToType, IDictionary<string, object> originalValueSource, OriginalValueRecord complexOriginalValueRecord)
    	{
    		...
    		{
    			object value;
    			string propertyPath = String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0}.{1}", propertyPathToType, property.Name);
    	...
    	private static EntityType GetCSpaceEntityType(this MetadataWorkspace workspace, Type type)
    	{
    		...
    		if(entityType == null)
    		{
    			throw new ArgumentException(String.Format(System.Globalization.CultureInfo.CurrentCulture, "Unable to find a CSpace type for type {0}", type.FullName));
    		}
    		return entityType;
    	}
