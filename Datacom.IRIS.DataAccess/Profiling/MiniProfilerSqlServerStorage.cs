...
{
    /// <summary>
    /// Based on SqlServerStorage that is shipped with MiniProfiler, with modified miniprofiler tables in database schema [profiling] 
    /// </summary>
    public class MiniProfilerSqlServerStorage : DatabaseStorageBase
    {
        /// <summary>
        /// Returns a new <see cref="SqlServerStorage"/>.
        /// </summary>
        public MiniProfilerSqlServerStorage(string connectionString)
        ...
        public override void Save(MiniProfiler profiler)
        {
            ...
            bool? isAjaxCall = null;
            bool displayOnScreen = GlobalUtils.GetAppSettingsValueAsBoolean(ConfigSettings.MiniProfilerDisplayOnScreen);

            //Hack: Ideally this should not be in the DA layer
            if (System.Web.HttpContext.Current != null)
            {
                httpRequestMethod = System.Web.HttpContext.Current.Request.HttpMethod;
                isAjaxCall = (System.Web.HttpContext.Current.Request.Headers["X-Requested-With"] == "XMLHttpRequest");
            }

            const string sql =
@"insert into Profiling.MiniProfilers
            (Id,
             Name,
             Started,
             MachineName,
...
select       @Id,
             ...
             @HttpMethod,
             @IsAjaxCall
where not exists (select 1 from Profiling.MiniProfilers where Id = @Id)"; // this syntax works on both mssql and sqlite
            ...
            using (var conn = GetOpenConnection())
            {
                var insertCount = conn.Execute(sql, new
                {
                    ...
                    HasUserViewed = displayOnScreen? profiler.HasUserViewed : true, //Set user has viewed to true if we don't want to display results to screen.
                ...
                {
                    conn.Execute("delete from Profiling.MiniProfilerClientTimings where MiniProfilerId = @Id", new { profiler.Id });
        ...
        protected virtual void SaveClientTiming(DbConnection conn, MiniProfiler profiler)
        {
            ...
            conn.Execute("insert into Profiling.MiniProfilerClientTimings(MiniProfilerId,Name,Start,Duration) values (@Id, @Name, @Start, @Duration)",
        ...
        {
            const string sql =
@"insert into Profiling.MiniProfilerTimings
            (Id,
             MiniProfilerId,
             ParentTimingId,
             Name,
...
values      (@Id,
        ...
        {
            const string sql =
@"insert into Profiling.MiniProfilerSqlTimings
            (Id,
             MiniProfilerId,
             ParentTimingId,
             ExecuteType,
...
values      (@Id,
        ...
        {
            const string sql =
@"insert into Profiling.MiniProfilerSqlTimingParameters
            (MiniProfilerId,
             ParentSqlTimingId,
             Name,
             DbType,
...
values      (@MiniProfilerId,
        ...
        {
            { typeof(MiniProfiler), "select * from Profiling.MiniProfilers where Id = @id" },
            { typeof(Timing), "select * from Profiling.MiniProfilerTimings where MiniProfilerId = @id order by RowId" },
            { typeof(SqlTiming), "select * from Profiling.MiniProfilerSqlTimings where MiniProfilerId = @id order by RowId" },
            { typeof(SqlTimingParameter), "select * from Profiling.MiniProfilerSqlTimingParameters where MiniProfilerId = @id" },
            { typeof(ClientTimings.ClientTiming), "select * from Profiling.MiniProfilerClientTimings where MiniProfilerId = @id"}
        };
        ...
        {
            //Do not Set user unviewed if we don't want to display results to screen.
            if (GlobalUtils.GetAppSettingsValueAsBoolean(ConfigSettings.MiniProfilerDisplayOnScreen))
            {
                using (var conn = GetOpenConnection())
                {
                    conn.Execute("update Profiling.MiniProfilers set HasUserViewed = 0 where Id = @id", new { id });
                }
            }
        }
        ...
        public override void SetViewed(string user, Guid id)
        {
            using (var conn = GetOpenConnection())
            {
                conn.Execute("update Profiling.MiniProfilers set HasUserViewed = 1 where Id = @id", new { id });
            }
        }
...
order  by Started";
        ...
        public override IEnumerable<Guid> List(int maxResults, DateTime? start = null, DateTime? finish = null, ListResultsOrder orderBy = ListResultsOrder.Decending)
        {
            ...
            var t = builder.AddTemplate("select top " + maxResults + " Id from Profiling.MiniProfilers /**where**/ /**orderby**/");
...
@"create table Profiling.MiniProfilers
  (
...
create table Profiling.MiniProfilerTimings
  (
...
create table Profiling.MiniProfilerSqlTimings
  (
     RowId                          integer primary key identity, -- sqlite: replace identity with autoincrement
     Id                             uniqueidentifier not null,
     MiniProfilerId                 uniqueidentifier not null,
...
create table Profiling.MiniProfilerSqlTimingParameters
  (
     MiniProfilerId    uniqueidentifier not null,
...
create table Profiling.MiniProfilerClientTimings
(
  MiniProfilerId    uniqueidentifier not null,
  Name nvarchar(200) not null,
  Start decimal(7,1),
